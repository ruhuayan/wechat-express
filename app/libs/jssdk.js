...
const fs = require('fs');
const jssdkUrl = require('./api').jssdk;
const debug = require('debug')('wechat-express:jssdk');
...
JSSDK.prototype = {
    ...
    getJsApiTicket: function (done) {
        ...
        if (typeof data.expireTime === 'undefined' || data.expireTime < time) {
            ...
            intance.getAccessToken(function (error, accessToken) {
                ...
                const url = jssdkUrl.ticket(accessToken); // `https://api.weixin.qq.com/cgi-bin/ticket/getticket?type=jsapi&access_token=${accessToken}`;
    ...
    getAccessToken: function (done) {
        ...
        if (typeof data.expireTime === 'undefined' || data.expireTime < time) {
            ...
            const url = jssdkUrl.token(this.appId, this.appSecret); 
            request.get(url, function (err, res, body) {
                if (err) {
